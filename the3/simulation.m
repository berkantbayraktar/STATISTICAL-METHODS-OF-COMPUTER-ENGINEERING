...
for i = 1:size  %Number of trials = size
    ...
    for j=1:poissrnd(lambda)+poissrnd(lambda)+poissrnd(lambda) %poisson random variable of caught fishes
        fishweight += weight(rand+rand+rand); % add weight of fishes that is caught 
    end;
...
[mean(overweight),mean(fishing),std(fishing)]
% a)Estimated probability ,b) estimated total weight ,c) estimated Std
